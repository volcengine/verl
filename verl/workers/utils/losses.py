...
def sft_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    if spec_logits is not None:
        # spec_logits shape: [n_predict, b, n, vocab_size]
        # We need to compute cross-entropy loss for each prediction head
        n_predict = spec_logits.shape[0]
        input_ids = data["input_ids"]
        loss_mask = data["loss_mask"]
        
        if pad_mode == DatasetPadMode.NO_PADDING:
            # Flatten input_ids and loss_mask
            input_ids_flat = input_ids.values()  # (total_nnz,)
            loss_mask_flat = loss_mask.values()  # (total_nnz,)
            
            spec_loss_total = 0.0
            for i in range(n_predict):
                # Get logits for head i
                # spec_logits[i] shape: (b, n, vocab_size)
                # In remove_padding mode, b=1, n=total_nnz
                logits_i = spec_logits[i].squeeze(0)  # (total_nnz, vocab_size)
                # Targets are shifted by i+2 (as in original logic)
                # Since we have flattened sequence, we need to shift each sample individually
                # We'll use torch.roll with appropriate shift
                ...
                # Weight by head importance (could be configurable)
            ...
            # metrics["speculator_loss"] = spec_loss_total.detach().item()
        else:
            ...
            for i in range(n_predict):
                ...
                # Mask out invalid positions using loss_mask
