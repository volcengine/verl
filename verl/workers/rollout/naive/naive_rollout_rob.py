...
class NaiveRolloutRob(BaseRollout):
    def __init__(
        ...
        self.module = OpenVLAForActionPrediction.from_pretrained(model_config["local_path"], trust_remote_code=True).to(
            torch.cuda.current_device()
        )
        self.module.vision_backbone.set_num_images_in_input(1)
        self.processor = PrismaticProcessor.from_pretrained(model_config["local_path"], trust_remote_code=True)
        dataset_statistics_path = os.path.join(model_config["local_path"], "dataset_statistics.json")
    ...
    def _generate_one_step(self, prompts: dict):
        ...
        max_prompt_length = prompts.get("prompt_length", self.config.prompt_length)
    ...
    def generate_sequences(self, prompts: TensorDict) -> DataProto:
        ...
        batch = TensorDict(
            {
                "input_ids": vla_input["input_ids"].cpu(),
                "attention_mask": vla_input["attention_mask"].cpu(),
                "pixel_values": vla_input["pixel_values"].cpu(),
