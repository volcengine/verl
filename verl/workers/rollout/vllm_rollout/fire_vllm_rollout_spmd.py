    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not self.use_fire_sampling:
            ...
            with self.update_sampling_params(**kwargs):
                ...
                new_idx_list = []
                for i in range(batch_size):
                    new_idx_list.append(idx_list[i] + list(output_0[0].outputs[i].token_ids))
                ...
                for i, output in enumerate(output):
                    for sample_id in range(len(output.outputs)):
                        response.append(output_0[i].outputs[sample_id].token_ids + output.outputs[sample_id].token_ids)
