...
GROUP_NUM_PER_ITER = NUM_ENVS_PER_ITER // GROUP_SIZE
...
def restructure_data_proto(data_proto: DataProto) -> list[TensorDict]:
    ...
    for stage_idx in range(STAGE_NUM):
        ...
        stage_tensordict = TensorDict(
            {
                "task_descriptions": final_tasks,
                "images_and_states": TensorDict(
                    {"full_image": final_images, "state": final_states},
                    batch_size=torch.Size([ENV_WORKERS_NUM * NUM_ENVS_PER_WORKER]),
                ),
            },
            batch_size=torch.Size([ENV_WORKERS_NUM * NUM_ENVS_PER_WORKER]),
        )
        stages_data_list.append(stage_tensordict)
...
async def run():
    ...
    state_ids = env_wg.get_all_state_ids()[0]
    ...
    reset_state_ids_tensordict = DataProto.from_single_dict({"state_ids": reset_state_ids_repeated})
    ...
    for _ in range(512 // 8):
        for stage_id in range(STAGE_NUM):
            if _ == 0:
                ...
                env_batch = env_obs_refs[stage_id].get()
                # obs = env_batch["obs"]
                obs = TensorDict(
                    {
                        "task_descriptions": env_batch.non_tensor_batch["task_descriptions"],
                        "images_and_states": TensorDict(
                            {"full_image": env_batch.batch["full_image"], "state": env_batch.batch["state"]},
                        ),
                    }
                )
    ...
    finish_ref = env_wg.finish_rollout()
    ray.get(finish_ref)
