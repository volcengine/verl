...
class HFModelConfig(BaseConfig):
    ...
    def __post_init__(self):
        ...
        # Get lora_rank from lora_adapter_path if not explicitly set
        if self.lora_adapter_path is not None:
            self.lora_rank = self._get_lora_rank_from_adapter_path(self.lora_adapter_path)

        # per model patch
        if getattr(self.hf_config, "model_type", None) == "kimi_vl":
            self.hf_config.text_config.topk_method = "greedy"

    def _get_lora_rank_from_adapter_path(self, adapter_path: str) -> int:
        """
