...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_param:
            logger.info(f"[update_actor] Loading actor params to GPU on rank {self.rank}")
            load_megatron_model_to_gpu(self.actor_module)
            log_gpu_memory_usage("After load actor params and grad during update_actor", logger=logger)
        if self._is_offload_optimizer:
            logger.info(f"[update_actor] Loading actor optimizer on rank {self.rank}")
        ...
        logger.info(f"[update_actor] Optimizer scheduler stepped on rank {self.rank}")
        ...
        if self._is_offload_param:
            logger.info(f"[update_actor] Offloading actor params from GPU on rank {self.rank}")
            offload_megatron_model_to_cpu(self.actor_module)
            log_gpu_memory_usage("After offload actor params and grad during update_actor", logger=logger)
        if self._is_offload_optimizer:
            logger.info(f"[update_actor] Offloading actor optimizer on rank {self.rank}")
            offload_megatron_optimizer(self.actor_optimizer)
