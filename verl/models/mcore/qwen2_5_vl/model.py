...
from ..util import AllGatherVisionEmbeddings, get_embeddings_on_this_cp_rank_thd, get_pos_ids_on_this_cp_rank_thd, get_vision_cp_data
...
class Qwen2_5VLModel(MegatronModule):
    ...
    def __init__(
        ...
        self.enable_vision_context_parallelism = getattr(vision_transformer_config, "enable_vision_context_parallelism", False)
    ...
    def forward(
        ...
        if self.pre_process:
            ...
            if self.llm_cp_size > 1 and self.enable_vision_context_parallelism:
                vision_data, vision_grid_thw, seqlen_on_cp_ranks = get_vision_cp_data(vision_data, vision_grid_thw, self.square_merge_size)
            ...
            if original_vision_grid_thw.shape[0] > 0 and self.llm_cp_size > 1 and self.enable_vision_context_parallelism:
            ...
            if self.llm_cp_size > 1:
                assert packed_seq_params is not None and packed_seq_params.qkv_format == "thd", "Only THD format is supported for context parallel"
        ...
        position_ids = get_rope_index_thd(input_ids, image_grid_thw=image_grid_thw, video_grid_thw=video_grid_thw, attention_mask=attention_mask, packed_seq_params=packed_seq_params)
