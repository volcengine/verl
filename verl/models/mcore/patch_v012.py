...
class PatchedDistributedOptimizer(DistributedOptimizer):
    ...
    def load_parameter_state_local(self, state_dict):
        ...
        if self.ddp_config.use_custom_fsdp:
            for model_chunk in self.model_chunks:
                ...
                for group_id, group in enumerate(pg_buffer.parameter_groups):
                    ...
                    this_group_state = state_dict.pop(group_key)
                    mbuf = group.master_weight_buffer
        ...
        for gbuf_idx, gbuf_range_maps in enumerate(self.gbuf_ranges):
            ...
            gbuf_state = state_dict.pop(gbuf_idx)

            assert len(gbuf_range_maps) == 1, "single dtype supported, for now."
            for dtype, gbuf_range_map_for_all_buckets in gbuf_range_maps.items():
                if dtype not in gbuf_state:
                    continue

                local_tensors = gbuf_state[dtype]
                ...
                for gbuf_range_map in gbuf_range_map_for_all_buckets:
                    for model_param, param_range_map in gbuf_range_map["param_map"].items():
                        ...
                        for key in tensors:
                            if key not in local_tensors or local_tensors[key] is None:
                                continue

                            for tensor_ref in local_tensors[key]:
                                if tensor_ref is not None and tensor_ref["start"] == gbuf_local_start and tensor_ref["end"] == gbuf_local_end:
    ...
    def load_sharded_parameter_state(self, filename: str):
        ...
        try:
            # Utilize memory mapping if available (PyTorch 2.1+)
