...
def apply_patch_mrope():
    ...
    def patch_multimodal_rotary_embedding_forward(self, position_ids: torch.Tensor, mrope_section):
        ...
        if not self.rotary_interleaved:
            ...
            bs = freqs.shape[1]
            emb = torch.stack((freqs.view(3, bs, -1, 1), freqs.view(3, bs, -1, 1)), dim=-1).view(3, bs, freqs.shape[0], -1)
        ...
        # mrope in mcore 0.12 can only support sbhd format context parallel, but qwen2_5_vl in verl use thd format. we need to disable it here.
