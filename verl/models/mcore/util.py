...
def preprocess_packed_seqs(input_ids: torch.Tensor, attention_mask: torch.Tensor, pre_process: bool = True, bypass_cp_shard: bool = False) -> tuple[torch.Tensor, PackedSeqParams]:
    """
...
class AllGatherLanguageOutputs(torch.autograd.Function):
    ...
    def forward(ctx, input, packed_seq_params):
        ...
        if packed_seq_params is None or packed_seq_params.qkv_format == "sbhd":
            ...
            ctx.seqlen = seqlen
            output = torch.zeros((*input.shape[:seq_dim], seqlen, *input.shape[seq_dim + 1 :]), device=input.device, dtype=input.dtype)
...
class AllGatherVisionEmbeddings(torch.autograd.Function):
    ...
    def forward(ctx, input, seqlens_on_cp_ranks):
        ...
        for i in range(len(seqlens_on_cp_ranks)):
            o = torch.zeros((seqlens_on_cp_ranks[i].sum(), *input.shape[1:]), device=input.device, dtype=input.dtype, layout=input.layout)
            outputs.append(o)
        torch.distributed.all_gather(outputs, input, group=mpu.get_context_parallel_group())
        cp_rank = mpu.get_context_parallel_rank()
        ctx.cp_rank = cp_rank
