    ...
    def _eviction_worker(self):
        while True:
            ...
            if was_inserted and self._size_in_bytes > self._cache_size_in_bytes:
                ...
                # delete could have already occurred, which makes eviction unnecessary), if still exceeds limit, execute.
                """
                After push, update the work queue to calculate the size. If the size exceeds, then initiate an eviction
                task. When the eviction task is executed, check the current bytes -> if doesn't exceed anymore, skip. Otherwise,
                after all eviction in a batch has executed, if still exceeds, initiate another eviction task.
                """
