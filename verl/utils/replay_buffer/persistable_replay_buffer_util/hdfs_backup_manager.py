...
class HDFSBackupManager:
    ...
    def _upload_to_hdfs_loop(self):
        ...
        local_rocksdb_path = f"{self._db_path}.zip"  # TODO: Maybe don't put them as local variables, since shared with task_processor's take snapshot method
        ...
        while True:
            self._upload_trigger.wait()  # sleep, until the upload trigger is set
            self._upload_trigger.clear()  # after waking up, unset the upload trigger so that it will sleep again on the next wait in the loop.
