...
class TaskProcessor:
    ...
    def _execute_p0_q(self, task_batch):
        ...
        write_batch = WriteBatch()  # WriteBatch size here doesn't exceed batch_limit (len(task_batch))
        # with self._p0_index_lock:
        for key, net_ops in net_operations.items():
            for task in net_ops:
                ...
                if task.type == TaskType.PUSH:
                    ...
                    self._execute_delete_operation(key, write_batch)
                # with self._p0_index_lock:
                self._p0_index[key].remove(task)
                if not self._p0_index[key]:  # empty
                    self._p0_index.pop(key)
                self.release_q_lock()
        # self._db.write(write_batch)  # NOTE: 有一段时间p0 index, self._db, self._p0_index都没有task。get可能拿不到
    ...
    def _execute_p1_q(self, task_batch):
        ...
        for task in task_batch:
            if task.type == TaskType.EVICT:
                self._execute_evict_operation(write_batch)
            elif task.type == TaskType.POPULATE:
                self._execute_populate_operation(task.key, write_batch)
        # self._db.write(write_batch)   # NOTE: evict -> 有一段时间self._db, lrucache都没有task，因为用了write batch。get可能拿不到
    ...
    def _execute_delete_operation(self, key, write_batch):
        self._cache.delete(key)
        self._db.delete(to_bytes(key))
        # write_batch.delete(to_bytes(key))
        for sampler in self._samplers:
            sampler.remove_index(key)

    def _execute_evict_operation(self, write_batch):
        ...
        self._add_to_db(key_lru, value_lru, write_batch)
        self._cache.delete(key_lru)  # size calculation in lru_cache eviction

    def _add_to_db(self, key, value, write_batch):
        ...
        # TODO: Can delete merging with existing value because cache is write-back, so there isn't existing
        # value in rocksdb when we are evicting from cache. (same key only exists on one side)
        ...
        # write_batch.add(key_encoded, serialized_value)

    def _execute_populate_operation(self, key, write_batch):
        ...
        # write_batch.delete(key_encoded)
