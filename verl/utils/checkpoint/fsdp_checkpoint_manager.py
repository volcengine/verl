...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def convert_lora_state_dict(self, state_dict: dict) -> dict:
        ...
        log_with_rank(
            "Converting state dict keys from original model format to LoRA/PEFT format", rank=self.rank, logger=logger
        )
        ...
        for key, value in state_dict.items():
            ...
            if key.startswith("model.") and (".weight" in key or ".bias" in key):
                ...
                if any(
                    layer_type in key
                    for layer_type in ["embed_tokens", "norm", "input_layernorm", "post_attention_layernorm"]
                ):
                ...
                # Handle linear layers that become LoRA layers
                    ...
                    proj_type in key
                    for proj_type in ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]
                ):
