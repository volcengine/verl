...
def call_sandbox_api(sandbox_fusion_url: str, code: str, stdin: str, compile_timeout: int, run_timeout: int, language: str = "python") -> Tuple[Optional[Dict[str, Any]], Optional[str]]: # <-- Remove request_id parameter
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            logger.error(f"{last_error} on attempt {attempt + 1}/{MAX_RETRIES}")
            ...
            logger.error(f"{last_error}. Raw response: {raw_response_text}")
    ...
    logger.error(f"{log_prefix}Sandbox API call failed after {MAX_RETRIES} attempts. Last error: {last_error}") # <-- Use internal log_prefix
...
def _process_single_case(
    ...
    if error_msg:
        ...
        logger.error(f"Case {case_index}: code: {generation}")
