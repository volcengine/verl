...
async def _query_openai_with_semaphore(
    semaphore: asyncio.Semaphore, client: AsyncOpenAI, sequence_str: str, config: Dict[str, Any]
) -> float:
    """
...
async def _query_openai_async(
    client: AsyncOpenAI, sequence_str: str, config
) -> float:
    """
...
async def process_data_async(data_source: List[str], solution_str: List[str], ground_truth: List[str], extra_info: List[Dict[str, Any]], config) -> torch.Tensor:
    """
