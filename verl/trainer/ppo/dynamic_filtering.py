"""
Dynamic filtering module for PPO training.
"""
...
class DynamicFilterManager:
    ...
    def apply_filter(self, batch: DataProto) -> tuple[int, List[int]]:
        ...
        for uid, metric_val in zip(uids, batch.non_tensor_batch[self.metric]):
        ...
        if not self.custom_filter_func:
            raise ValueError(
                ...
                "For the original mixed rewards filter, use 'verl.trainer.ppo.dynamic_filtering.keep_mixed_reward'"
            )
...
def keep_mixed_reward(metric_vals: List[Union[float, int]], **kwargs) -> bool:
