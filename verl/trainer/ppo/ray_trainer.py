...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty="kl"):
    response_mask = compute_response_mask(response_ids=data.batch["responses"], attention_mask=data.batch["attention_mask"])
...
def calc_mini_batch_loss_token_nums(batch: DataProto, traj_mini_bsz: int, num_dp_ranks: int) -> list[int]:
    ...
    """
    response_mask = compute_response_mask(response_ids=batch.batch["responses"], attention_mask=batch.batch["attention_mask"])
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(token_level_rewards=data.batch["token_level_rewards"], response_mask=response_mask, index=data.non_tensor_batch["uid"])
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        ...
        advantages, returns = core_algos.compute_reinforce_plus_plus_outcome_advantage(token_level_rewards=data.batch["token_level_rewards"], response_mask=response_mask, gamma=gamma)
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.REMAX:
        advantages, returns = core_algos.compute_remax_outcome_advantage(token_level_rewards=data.batch["token_level_rewards"], reward_baselines=data.batch["reward_baselines"], response_mask=response_mask)

        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.RLOO:
        advantages, returns = core_algos.compute_rloo_outcome_advantage(token_level_rewards=data.batch["token_level_rewards"], response_mask=response_mask, index=data.non_tensor_batch["uid"])
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    else:
