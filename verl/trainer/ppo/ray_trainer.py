...
class RayPPOTrainer(object):
    ...
    def _validate_config(self):
        ...
        # Actor
        # check if train_batch_size is divisible by ppo_mini_batch_size
        # if NOT dynamic_bsz, we must ensure:
        #    ppo_mini_batch_size is divisible by ppo_micro_batch_size
        #    ppo_micro_batch_size * sequence_parallel_size >= n_gpus
        if not config.actor_rollout_ref.actor.use_dynamic_bsz:
            assert config.data.train_batch_size % config.actor_rollout_ref.actor.ppo_mini_batch_size == 0
        ...
        # critic
        if self.use_critic and not config.critic.use_dynamic_bsz:
            assert config.data.train_batch_size % config.critic.ppo_mini_batch_size == 0
