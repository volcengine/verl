...
def compute_mismatch_metrics_batch(batch: DataProto) -> dict[str, Any]:
    """
    Compute training-inference mismatch metrics from batch data.

    This provides a centralized way to compute mismatch metrics (KL divergence, perplexity, etc.)
    between rollout policy (e.g., vLLM) and training policy (e.g., FSDP) at the trainer level,
    rather than inside individual workers.

    The function is designed to match the pattern of other metrics functions in this module:
    ...
    Args:
        batch: A DataProto object containing:
            - batch['old_log_probs']: Log probs from training policy (required)
            - batch['rollout_log_probs']: Log probs from rollout policy (optional)
            - batch['attention_mask'] or batch['response_mask']: Token mask

    Returns:
        Dictionary of mismatch metrics with 'mismatch/' prefix:
            - mismatch/training_ppl: Perplexity of training policy
            - mismatch/training_log_ppl: Log perplexity of training policy
            - mismatch/kl: KL divergence (if rollout_log_probs available)
            - mismatch/k3_kl: K3 KL estimator (if rollout_log_probs available)
            - mismatch/rollout_ppl: Perplexity of rollout policy (if rollout_log_probs available)
            - mismatch/log_ppl_diff: Difference in log perplexities (if rollout_log_probs available)
            - mismatch/ppl_ratio: Ratio of training PPL to rollout PPL (if rollout_log_probs available)

    Example:
        >>> # In ray_trainer.py
        >>> metrics.update(compute_data_metrics(batch=batch, use_critic=self.use_critic))
        >>> metrics.update(compute_timing_metrics(batch=batch, timing_raw=timing_raw))
        >>> metrics.update(compute_mismatch_metrics_batch(batch=batch))  # Add this line

    Reference:
        When Speed Kills Stability: https://yingru.notion.site/When-Speed-Kills-Stability-271211a558b7808d8b12d403fd15edda
    """
    # Early return if no log probs available
    if "old_log_probs" not in batch.batch:
        return {}

    from verl.trainer.ppo.mismatch_helper import compute_mismatch_metrics

    # Extract response mask
    response_mask = None
    if "response_mask" in batch.batch:
        ...
        # Use full attention mask if no responses specified
        response_mask = batch.batch["attention_mask"]
    else:
        # Cannot compute metrics without mask
        return {}

    # Get rollout log probs if available
    rollout_log_prob = batch.batch.get("rollout_log_probs", None)

    # Compute mismatch metrics using the helper function from mismatch_helper
    ...
    # Add 'mismatch/' prefix to all metrics for consistency
    return {f"mismatch/{key}": value for key, value in mismatch_metrics.items()}
