...
def compute_policy_loss(
    ...
    Args:
        ...
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        loss_agg_mode: (str) choices: "token-mean" / "seq-mean-token-sum" / "seq-mean-token-mean" / "seq-mean-token-sum-norm"
            "token-mean" is the default behavior
        cliprange: (float)
...
def compute_value_loss(vpreds: torch.Tensor, returns: torch.Tensor, values: torch.Tensor, response_mask: torch.Tensor, cliprange_value: float, loss_agg_mode: str):
    ...
    vf_loss = agg_loss(loss_mat=0.5 * torch.max(vf_losses1, vf_losses2), loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
