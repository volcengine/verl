...
@register_policy_loss("sft")  # type: ignore[arg-type]
def compute_sft_loss(
    ...
    """
    Compute the supervised fine-tuning loss (cross entropy).
    Args:
        old_log_prob (torch.Tensor):
            Log-probabilities of actions under the old policy, shape (batch_size, response_length).
        log_prob (torch.Tensor):
    ...
    loss = agg_loss(
        loss_mat=cross_entropy, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, **config.global_batch_info
    )
    return loss, {}
