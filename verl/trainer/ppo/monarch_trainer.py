...
"""
FSDP PPO Trainer with Monarch-based single controller.
...
from monarch.actor import current_rank, proc_mesh, ProcMesh
...
async def create_mast_proc_mesh(
    ...
    def env():
        ...
        os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str(rank) for rank in ranks)
        print(f"{os.environ['CUDA_VISIBLE_DEVICES']=}")

    pm = ProcMesh.from_alloc(alloc, env)
    # print("syncing workspace")
    # await pm.sync_workspace()
    # print("done syncing workspace")
    return pm
...
class ResourcePoolManager:
    ...
    def create_resource_pool(self):
        ...
        for resource_pool_name, process_on_nodes in self.resource_pool_spec.items():
            ...
            ), "Monarch backend currently expects the same number of processes on every node"
            # TODO make this generic for non-procmesh
            num_procs = process_on_nodes[0]
            if self.mast_job:
                pm = asyncio.run(
                    create_mast_proc_mesh(
...
class MonarchPPOTrainer:
    ...
    def __init__(
        ...
        """
        Initialize distributed PPO trainer with Monarch backend.
        Note that this trainer runs on the driver process on a single CPU/GPU node.
        Args:
            config: Configuration object containing training parameters.
            tokenizer: Tokenizer used for encoding and decoding text.
            role_worker_mapping (dict[Role, WorkerType]): Mapping from roles to worker classes.
            resource_pool_manager (ResourcePoolManager): Manager for Monarch resource pools.
            monarch_worker_group_cls (MonarchWorkerGroup, optional): Class for worker groups. Defaults to MonarchWorkerGroup.
    ...
    def init_workers(self):
        """Initialize distributed training workers using Monarch backend.
        Creates:
