...
class FSDPSFTTrainer:
    def __init__(
        ...
        self.engine.intialize()


    def create_engine_config(self, config):
        model_config = engine_cfg.get_model_config(
            config.model,
            module_type="causal_lm",
            path=config.model.partial_pretrain)
        
        optim_config = engine_cfg.get_optim_config(
            config.optim,
            grad_clip=config.optim.clip_grad,
            lr_warmup_steps_ratio=config.optim.warmup_steps_ratio,
            lr_scheduler_style=config.optim.lr_scheduler)
        
        system_config = engine_cfg.get_system_config(
        ...
        ckpt_config = engine_cfg.get_checkpoint_config(config.trainer.checkpoint)
        

        raise ValueError

        ret = engine_cfg.get_engine_config(config,
                                            model_config,
                                            optim_config,
                                            system_config,
                                            ckpt_config,
