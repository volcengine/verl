...
"""A lightweight single-file FSDP SFT trainer.

This implementation focuses on simplicity and currently omits advanced features
such as model FLOPs utilization (MFU) calculation and full validation loops.
...
class FSDPSFTTrainer:
    """Simple trainer for supervised fine-tuning using FSDP."""
    ...
    def __init__(
        ...
        # Checkpoint management is not yet integrated.  Models are saved
        # directly to the output directory after each epoch.
        if self.device_mesh.get_rank() == 0:
            print(self.config)

    def _normalize_config_bsz(self) -> None:
        """Normalize batch size settings based on data parallel size."""
        dp_size = (
    ...
    def _build_model_optimizer(self) -> None:
        """Construct the model, FSDP wrapper and optimizer."""
        # Currently models are loaded from partially pre-trained checkpoints.
        # Support for random initialization or loading directly from sharded
    ...
    def _compute_loss_and_backward(
        self, batch: TensorDict, do_backward: bool = True
    ) -> torch.Tensor:
        """Compute loss and optionally perform backward propagation."""
        use_sp = (
    ...
    def training_step(self, batch: TensorDict) -> dict[str, float]:
    ...
    def validation_step(self, batch: TensorDict) -> torch.Tensor:
    ...
    def save_checkpoint(self, step: int) -> None:
        """Persist model and tokenizer state for the given step."""
        # save checkpoint
    ...
    def fit(self) -> None:
        ...
        # Tracking currently uses the generic :class:`Tracking` helper.  A
        # more feature complete logging solution may be integrated later.
        if rank == 0:
            tracking = Tracking(
        ...
        # Uploading checkpoints to HDFS can be slow, so the checkpoint manager
...
def create_sft_dataset(
    ...
    """Create the SFT dataset used for fine-tuning.
