...
# Ulysses SP=2, DP=2 on 4 GPUs; vLLM rollout TP=1 (simple, head-divisible for all models).
# Produces a timestamped log under $HOME.
...
LOG_FILE="$HOME/uly_sp2_dp2_vllm_tp1_$(date +'%Y%m%d_%H%M%S').log"

echo "--- ULY SP=2, DP=2, vLLM TP=1 | logging to: $LOG_FILE ---"

# Build Hydra args as array for robust quoting
ARGS=(
  ...
  # Actor (DeepSpeed) with Ulysses SP=2, total 4 GPUs -> DP ≈ 2
  ...
  # vLLM rollout with TP=1 (DP for rollout = world/TP)
