...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_param and data.meta_info.get("load_flag", True):
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer and data.meta_info.get("load_flag", True):
        ...
        with self.ulysses_sharding_manager:
            ...
            if not data.meta_info.get("skip_lr_strategy", False):
        ...
        if self._is_offload_param and data.meta_info.get("offload_flag", True):
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor", logger=logger)
        if self._is_offload_optimizer and data.meta_info.get("offload_flag", True):
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(
                tensors={key: output, "entropys": entropys},
