...
class AsyncRewardAgentTrainer:
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix="global_seqlen"):
        ...
        if self.config.actor_rollout_ref.actor.strategy == "megatron":
            dp_size = self.actor_rollout_wg.dp_size
        else:
    ...
    def _rollout_and_request_rewards(self, continuous_iterator):
        ...
        if self.config.actor_rollout_ref.actor.policy_loss.get("loss_mode", "vanilla") == "decoupled":
            ...
            with marked_timer("behav_log_prob", timing_raw, color="blue"):
                batch.meta_info["cal_behav_log_prob"] = True
                behav_log_prob = self.actor_rollout_wg.compute_log_prob(batch)
                behav_log_prob.batch.pop("entropys")
    ...
    def fit(self):
        ...
        self.reward_agent = RayAsyncRewardAgent.remote(self.config, self.tokenizer)
        ...
        while self.global_steps <= self.total_training_steps:
            ...
            with marked_timer("step", timing_raw):
                ...
                if last_timing_raw:
                    timing_raw.update(last_timing_raw)
                ...
                while current_mini_batch_num < total_mini_batch_num:
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.config.algorithm.use_kl_in_reward:
                            mini_batch, kl_metrics = apply_kl_penalty(
                                mini_batch, kl_ctrl=self.kl_ctrl_in_reward, kl_penalty=self.config.algorithm.kl_penalty
                            )
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with marked_timer("update_actor", timing_raw, color="red"):
                            mini_batch.meta_info["multi_turn"] = self.config.actor_rollout_ref.rollout.multi_turn.enable
                ...
                batch_metrics = reduce_metrics(batch_metrics)
        ...
        ray.get(self.reward_agent.shutdown.remote())
