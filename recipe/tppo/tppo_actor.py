...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        if 'left_pad_len' in micro_batch.keys():# NOTE(zht)
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                full_output = full_log_probs.squeeze(-1)
                if 'left_pad_len' in micro_batch.keys(): # NOTE(zht)
                    log_probs = torch.zeros_like(full_output)
                    for idx, r in enumerate(response_length):
                        log_probs[idx, :r] = full_output[idx, -r - 1: -1]
                else:
                    log_probs = full_output[:, -response_length - 1 : -1]  # [batch_size, response_length]
    ...
    def update_policy(self, data: DataProto):
        ...
        if 'window_rounds' in data.batch.keys(): # ADD(zht)
            select_keys += ['left_pad_len', 'actual_prompt_len', 'is_finished', 'rounds_eos_mask', 'window_rounds', 'token_level_scores']

        batch = data.select(batch_keys=select_keys).batch

        ###################################################################################
        ###################################################################################
        response_length = batch['responses'].size(1) # NOTE(zht)
        batch_full_token_count_mask = batch['attention_mask'][:, -response_length:]
        if 'overlong_mask' in batch.keys():
            batch_full_token_count_mask *= batch['overlong_mask'].unsqueeze(-1)
        if 'acc_mask' in batch.keys():
            batch_full_token_count_mask *= batch['acc_mask'].unsqueeze(-1)
        batch_full_token_count = max(1, batch_full_token_count_mask.sum().item())
        # compute batch lm full token count
        batch_rounds_eos_mask = batch.get('rounds_eos_mask', None)
        if batch_rounds_eos_mask is not None:
            batch_eos_ids = batch['eos_ids'].unsqueeze(1)
            batch_raw_scores = batch['token_level_scores']
            batch_scores = torch.gather(batch_raw_scores, 1, batch_eos_ids)
            batch_mask = (batch_scores > 0).repeat(1, batch_rounds_eos_mask.shape[1])
            batch_mask &= batch_rounds_eos_mask.bool()
            batch_lm_full_token_count = max(1, batch_mask.sum().item())
        else:
            batch_lm_full_token_count = batch_full_token_count
        ###################################################################################
        ###################################################################################
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                ############################################################################################
                # compute minibatch full token count  # NOTE(zht)
                ...
                if 'acc_mask' in mini_batch.keys():
                    mini_batch_full_token_count_mask *= mini_batch['acc_mask'].unsqueeze(-1)
                mini_batch_full_token_count = max(1, mini_batch_full_token_count_mask.sum().item())
                
                mini_batch_rounds_eos_mask = mini_batch.get('rounds_eos_mask', None)
                if mini_batch_rounds_eos_mask is not None:
                    mini_batch_eos_ids = mini_batch['eos_ids'].unsqueeze(1)
                    mini_batch_raw_scores = mini_batch['token_level_scores']
                    mini_batch_scores = torch.gather(mini_batch_raw_scores, 1, mini_batch_eos_ids)
                    mini_batch_mask = (mini_batch_scores > 0).repeat(1, mini_batch_rounds_eos_mask.shape[1])
                    mini_batch_mask &= mini_batch_rounds_eos_mask.bool()
                    mini_batch_lm_full_token_count = max(1, mini_batch_mask.sum().item())
                else:
                    mini_batch_lm_full_token_count = mini_batch_full_token_count
                ############################################################################################
                ...
                for data in micro_batches:
                    ...
                    ##################################################################################################
                    eos_ids = data.get('eos_ids', None) # NOTE(zht): microdata actually
                    overlong_mask = data.get('overlong_mask', None)
                    acc_mask = data.get('acc_mask', None)
                    single_eos_ids = eos_ids.clone()
                    if data.get('window_rounds', None) is not None:
                        total_len = log_prob.size(1)
                        window_response_length = self.config.window_response_length
                        single_log_prob = torch.zeros_like(log_prob)
                        for idx, window_round in enumerate(data['window_rounds']):
                            single_log_prob[idx, :total_len-window_round * window_response_length] = log_prob[idx, window_round * window_response_length:]
                            single_eos_ids[idx] -= window_round * window_response_length
                        single_log_prob = single_log_prob[:, :window_response_length]
                    else:
                        single_log_prob = log_prob
                    
                    if self.config.policy_loss.loss_mode == "tppo": # loss_mode must be tppo !!!!!!!
                        from recipe.tppo.tppo_algos import compute_policy_loss
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(
                            old_log_prob=old_log_prob,
                            log_prob=single_log_prob,
                    ...
                    if entropy_coeff != 0:
                        from recipe.tppo.tppo_algos import agg_loss
                    ...
                    if self.config.use_kl_loss:
                        from recipe.tppo.tppo_algos import compute_kl_loss
                        ref_log_prob = data["ref_log_prob"]
                        kl_loss = compute_kl_loss(log_prob, ref_log_prob, response_mask, self.config.kl_loss_type, loss_agg_mode)
                    ...
                    ##################################################################################
                    # lm_loss # NOTE(zht): need to add 'token_level_scores'
                    lm_loss_weight = self.config.lm_loss_weight
                    if lm_loss_weight > 0:
                        from recipe.tppo.tppo_algos import compute_lm_loss
                        rounds_eos_mask = data.get('rounds_eos_mask', None)
                        lm_loss = compute_lm_loss(
                            log_prob=log_prob,
                            raw_score=data['token_level_scores'],
                            eos_ids=eos_ids,
                            rounds_eos_mask=rounds_eos_mask,
                            loss_average_method=loss_agg_mode,
                        )
                    ...
                    if loss_agg_mode=='minibatch':
                        loss = (pg_loss - entropy_loss*entropy_coeff + kl_loss*kl_loss_coef) / mini_batch_full_token_count + lm_loss_weight*lm_loss / mini_batch_lm_full_token_count
                    elif loss_agg_mode=='batch':
                        loss = ((pg_loss - entropy_loss*entropy_coeff + kl_loss*kl_loss_coef) / batch_full_token_count + lm_loss_weight*lm_loss / batch_lm_full_token_count)*len(dataloader)
                    else:
