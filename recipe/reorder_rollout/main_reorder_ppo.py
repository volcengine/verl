...
from recipe.reorder_rollout.reorder_trainer import ReorderRolloutTrainer
...
def main(config):
    run_reorder_ppo(config)


# Define a function to run the PPO-like training process
def run_reorder_ppo(config) -> None:
...
class TaskRunner:
    def run(self, config):
        ...
        val_reward_fn = load_reward_manager(
            config, tokenizer, num_examine=100, **config.reward_model.get("reward_kwargs", {})
        )
