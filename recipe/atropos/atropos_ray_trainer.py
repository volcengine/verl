...
from .atropos_api_client import AtroposAPIClient, AtroposConfig, AtroposAPIError
    ...
    def __init__(self, config, role_worker_mapping, resource_pool_manager):
        ...
        if not self.atropos_client.test_connectivity():
            raise AtroposAPIError(
                f"Cannot connect to Atropos API at {atropos_config.api_url}. "
                "Please ensure the Atropos server is running."
            )
            
        # Register trainer with Atropos
        try:
            success = self.atropos_client.register_trainer(
                starting_step=0,
