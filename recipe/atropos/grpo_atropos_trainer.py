...
class RayGRPOAtroposTrainer(RayPPOTrainer):
    ...
    def __init__(self, config, role_worker_mapping, resource_pool_manager):
        ...
        self.atropos_client = AtroposEnvironmentClient(api_url=atropos_config.get("api_url", "http://localhost:8000"), timeout=atropos_config.get("timeout", 30))

        # Check connectivity
        if not self.atropos_client.health_check():
            raise ConnectionError(f"Cannot connect to Atropos at {self.atropos_client.api_url}. Please start Atropos server first.")

        # Initialize GRPO computer
        self.grpo_computer = AtroposGRPOComputer(self.atropos_client)

        # GRPO-specific config
        self.grpo_config = {"kl_coef": config.algorithm.get("kl_coef", 0.1), "use_token_level_overrides": config.algorithm.get("use_token_level_overrides", True), "group_size": config.algorithm.get("group_size", 8), "normalize_advantages": config.algorithm.get("normalize_advantages", True)}
    ...
    def _get_prompts_from_atropos(self, batch_size: int) -> Optional[DataProto]:
        ...
        for prompt in prompts:
            tokens = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=self.config.data.max_prompt_length)
        ...
        # Add metadata
        non_tensor_batch = {"raw_prompts": prompts, "atropos_metadata": metadata, "group_ids": result.get("group_ids", [])}
    ...
    def training_step(self, batch_dict: Dict) -> Dict:
        ...
        with marked_timer("training_step", timing_raw):
            ...
            with marked_timer("generation", timing_raw):
                gen_batch = batch.pop(batch_keys=["input_ids", "attention_mask"], non_tensor_batch_keys=["raw_prompts"])

                # Generate multiple responses per prompt for GRPO
                gen_output = self.actor_rollout_wg.generate_sequences(gen_batch, n=self.grpo_config["group_size"])
