...
class AtroposEnvironmentClient:
    ...
    def submit_responses_and_get_advantages(
        ...
        try:
            # Submit to Atropos
            response = self.session.post(
                f"{self.api_url}/evaluate_and_compute_advantages",
                json=payload,
                timeout=self.timeout
            )
...
class AtroposGRPOComputer:
    ...
    def compute_advantages_with_overrides(
        ...
        metadata: Dict[str, Any],
        kl_coef: float = 0.1,
        use_token_level_overrides: bool = True
        ...
        result = self.client.submit_responses_and_get_advantages(
        ...
        if result is None:
            ...
            return self._compute_standard_grpo_advantages(
                old_log_probs, ref_log_probs, kl_coef
            ), {}
    ...
    def _compute_standard_grpo_advantages(
        self,
        old_log_probs: torch.Tensor,
        ref_log_probs: Optional[torch.Tensor],
        kl_coef: float
...
def create_atropos_grpo_trainer(config):
    ...
    if not client.health_check():
        raise ConnectionError(
            ...
            "Please ensure Atropos server is running."
        )
    ...
    return {
        "client": client,
        "grpo_computer": grpo_computer,
        "config": {
            "use_token_level_overrides": config.get("use_token_level_overrides", True),
            "kl_coef": config.get("kl_coef", 0.1),
            "fallback_to_standard": config.get("fallback_to_standard", True)
        }
    }