...
class AtroposAPIClient:
    ...
    def test_connectivity(self) -> bool:
        ...
        try:
            response = requests.get(f"{self.config.api_url}/status", timeout=self.config.timeout)
    ...
    def register_trainer(self, starting_step: int = 0, num_steps: int = 1000) -> bool:
        ...
        try:
            response = requests.post(f"{self.config.api_url}/register", json=registration_data, timeout=self.config.timeout)
    ...
    def get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(f"{self.config.api_url}/envs", timeout=self.config.timeout)
            if response.status_code == 200:
    ...
    def submit_scored_data(
        ...
        """
        scored_data = {"tokens": tokens, "masks": masks, "scores": scores, "ref_logprobs": ref_logprobs, "messages": messages, "advantages": advantages, "overrides": overrides, "group_overrides": group_overrides, "images": images}

        # Remove None values
        scored_data = {k: v for k, v in scored_data.items() if v is not None}

        try:
            response = requests.post(f"{self.config.api_url}/scored_data", json=scored_data, timeout=self.config.timeout)
    ...
    def retrieve_batch(self) -> Optional[List[Dict[str, Any]]]:
        ...
        for attempt in range(self.config.retry_attempts):
            ...
            try:
                ...
                status_response = requests.get(f"{self.config.api_url}/status", timeout=5)
                ...
                # Try to get batch
                response = requests.get(f"{self.config.api_url}/batch", timeout=self.config.timeout)
    ...
    def process_rollout_data(self, prompts: torch.Tensor, responses: torch.Tensor, tokenizer, scores: Optional[List[float]] = None, log_probs: Optional[torch.Tensor] = None, ref_model=None) -> Optional[Dict[str, Any]]:
        """
        ...
        # Submit to Atropos
        success = self.submit_scored_data(tokens=tokens_list, masks=masks_list, scores=scores, ref_logprobs=ref_logprobs_list if ref_model is not None else None)
    ...
    def _compute_default_scores(self, responses: torch.Tensor, log_probs: Optional[torch.Tensor], tokenizer) -> List[float]:
    ...
    def _extract_advantages_from_batch(self, batch: List[Dict[str, Any]], expected_batch_size: int, seq_len: int) -> Dict[str, Any]:
        ...
        return {"advantages": advantages, "batch_data": batch, "processed_count": min(len(batch), expected_batch_size)}
