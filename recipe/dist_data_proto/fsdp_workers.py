...
from recipe.dist_data_proto.rollout_worker import RolloutWorker
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        from recipe.dist_data_proto.dp_actor import DataParallelPPOActor
    ...
    def update_actor(self, data: DistDataProto):
    ...
    def generate_sequences(self, prompts_ddp: DataProto | DistDataProto):
        if type(prompts_ddp) is DistDataProto:
            prompts = prompts_ddp.select(device=get_device_id())  # NOTE(caiyunke.astra): Simple implementation without prefetch.
        else:
            prompts = prompts_ddp.to(get_device_id())
        ...
        if type(prompts_ddp) is DistDataProto:
            output_ddp = DistDataProto.from_dataproto(output, dataworker_handles=prompts_ddp.dataworker_handles)
            return output_ddp
        else:
            return output
    ...
    def compute_log_prob(self, data: DistDataProto):
    ...
    def compute_ref_log_prob(self, data: DistDataProto):
        if self._is_lora:
