...
def resample_of_correct(
    batch: DataProto, tokenizer: PreTrainedTokenizerFast, config: dict, do_sample=True, world_size=None
):
    ...
    assert min_zero_reward_trace_num + min_non_zero_reward_trace_num <= down_sample_to_n, (
        "Invalid down sampling configuration: "
        + f"{min_zero_reward_trace_num=}, {min_non_zero_reward_trace_num=}, {down_sample_to_n=}"
    )
    ...
    if do_sample and down_sample_to_n > 0:
        ...
        for uid in unique_uids:
            ...
            zero_reward_pairs = [
                (indice, penalty_weight)
                for indice, uid_reward, penalty_weight in zip(
                    indices, uid_rewards, penalty_weights[uid_mask], strict=False
                )
                if uid_reward <= 0
            ]
            non_zero_reward_pairs = [
                (indice, penalty_weight)
                for indice, uid_reward, penalty_weight in zip(
                    indices, uid_rewards, penalty_weights[uid_mask], strict=False
                )
            ...
            if (
                zero_reward_trace_num < min_zero_reward_trace_num
                and non_zero_reward_trace_num < min_non_zero_reward_trace_num
            ):
                pprint(
                    f"Total trace number before down sampling: {len(indices)},"
                    + f" smaller than {min_zero_reward_trace_num=} + {min_non_zero_reward_trace_num=}"
                )
                valid_mask[indices] = True
            else:
                ...
                choices = [
                    non_zero_reward_pair[0]
                    for non_zero_reward_pair in non_zero_reward_pairs[:non_zero_reward_trace_num]
