...
class MultiStepOffRayTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        self.async_rollout_manager.initialize()
        ...
        while self.global_steps <= self.total_training_steps:
            ...
            with marked_timer("step", timing_raw):
                ...
                with marked_timer("wait_prev_gen", timing_raw, color="red"):
                    batch, gen_batch_output = self.async_rollout_manager.prepare_batch(
                        continuous_iterator
                    )
            ...
            metrics.update(
                {
                    ...
                    "training/epoch": 1
                    + self.global_steps
                    // (len(continuous_iterator) // self.config.trainer.total_epochs),
                }
            )
            # collect metrics
            metrics.update(
