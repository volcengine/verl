...
from .ray_trainer import MultiStepOffRayTrainer


@hydra.main(
    config_path="config", config_name="one_step_off_ppo_trainer", version_base=None
)
def main(config):
    run_ppo(config)
...
def run_ppo(config) -> None:
    ...
    if (
        ...
        nsight_options = OmegaConf.to_container(
            config.global_profiler.tool_config.nsys.controller_nsight_options
        )
...
class TaskRunner:
    def run(self, config):
        ...
        ), "config.trainer.n_gpus_per_node must be greater than 0"
        assert config.trainer.nnodes > 0, "config.trainer.nnodes must be greater than 0"
        assert (
            config.rollout.n_gpus_per_node > 0
        ), "config.rollout.n_gpus_per_node must be greater than 0"
        ...
        trainer = MultiStepOffRayTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
