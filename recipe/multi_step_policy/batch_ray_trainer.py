...
class MultiStepOffRayTrainer(RayPPOTrainer):
    ...
    def __init__(
        ...
        self.stop_queue_thread = False
        self.continuous_iterator = None

        # 添加权重更新标志
        self.weights_updated = False  # 标记模型参数是否已更新
        self.global_steps = 0
    ...
    def sync_rollout_weights(self):
        """只在权重有更新时才执行同步操作"""
        if self.weights_updated or self.global_steps == 0:  # 初始步骤或权重已更新时同步
            if not self.hybrid_engine:
                self.actor_wg.sync_rollout_weights()
                ray.get(self.rollout_wg.sync_rollout_weights())
            self.weights_updated = False  # 重置更新标志
    ...
    def _async_gen_next_batch(self, continuous_iterator):
        ...
        # sync weights from actor to rollout (仅在更新后同步)
        self.sync_rollout_weights()
        # async generation
    ...
    def _queue_feeder(self):
        ...
        后台线程函数，负责填充队列
        """
        while not self.stop_queue_thread:
            try:
                # 生成下一个批次
                batch_future = self._async_gen_next_batch(self.continuous_iterator)
                ...
                # 将批次放入队列，会自动阻塞直到有空间
                self.batch_queue.put(batch_future.get(), block=True)

            except Exception as e:
                print(f"Error in queue feeder thread: {e}")
                if self.stop_queue_thread:
                    break
    ...
    def fit(self):
        ...
        # 初始化连续迭代器
        self.continuous_iterator = self._create_continuous_iterator()

        # 启动队列填充线程
        self.stop_queue_thread = False
        self.queue_thread = threading.Thread(target=self._queue_feeder, daemon=True)
        ...
        try:
            while True:
                ...
                # 从队列获取批次数据
                try:
                    batch_data_future = self.batch_queue.get(block=True, timeout=5.0)
                except queue.Empty:
                    # 如果队列为空且填充线程已结束，退出循环
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.use_critic:
                        with marked_timer("update_critic", timing_raw, color="pink"):
                            ...
                            self.weights_updated = True  # 标记权重已更新
