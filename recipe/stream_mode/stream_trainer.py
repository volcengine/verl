...
from recipe.stream_mode.utils import gen_next_batch
...
from verl.trainer.ppo.ray_trainer import ResourcePoolManager, Role, WorkerType, apply_kl_penalty, compute_advantage, compute_response_mask
...
class RayStreamPPOTrainer:
    ...
    def __init__(
        ...
        self.stream_mode = config.actor_rollout_ref.rollout.stream_mode
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        self.train_batch = self.config.data.get("gen_batch_size", self.config.data.train_batch_size)
        reader_batch = 1 if self.stream_mode else self.train_batch
        ...
        if self.stream_mode:
            assert self.config.trainer.total_training_steps is not None, "total_training_steps must be set when stream_mode is True"
    ...
    def init_workers(self):
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            from recipe.stream_mode.chat_scheduler.server_mgr import AsyncLLMServerManager
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if not self.async_rollout_mode:
                            ...
                            stop_iter, gen_batch_output, gen_batch, batch = self.async_generate_sequence(data_iter=data_iter, batch_size=self.train_batch, renew=renew)
                            renew = False
                            if stop_iter:
                                break
    ...
    def async_generate_sequence(self, data_iter: Iterable, batch_size, renew=False):
        self.async_rollout_manager.wake_up()
        if self.stream_mode:
            stop_iter, gen_batch_output, gen_batch, batch = self.async_rollout_manager.stream_generate_sequences(data_iter, batch_size=batch_size, renew=renew)
            if stop_iter:
