...
class _VocabParallelKLDivergence(torch.autograd.Function):
    ...
    def forward(ctx, vocab_parallel_logits, target_topk_logps, target_topk_indices):
        ...
        topk_indices_in_vocab_mask = ((target_topk_indices >= vocab_start_index) & (target_topk_indices < vocab_end_index))
        ...
        arange_1d = torch.arange(start=0, end=vocab_parallel_source_probs_2d.size(0), 
                                 device=vocab_parallel_source_probs_2d.device) # (b*s, )
        vocab_parallel_source_topk_probs_2d = vocab_parallel_source_probs_2d[arange_1d.unsqueeze(-1), 
                                                                             vocab_parallel_target_topk_indices.view(-1, topk)] # (b*s, topk)
        vocab_parallel_source_topk_probs = vocab_parallel_source_topk_probs_2d.view(target_topk_logps_origin_shape) # (b, s, topk)
        ...
        per_token_kl_loss = torch.sum(
            vocab_parallel_target_topk_probs * (vocab_parallel_target_topk_logps - vocab_parallel_source_topk_logps), dim=-1) # (b, s)
        ...
        ctx.save_for_backward(vocab_parallel_source_probs, vocab_parallel_target_topk_probs, 
    ...
    def backward(ctx, grad_output):
        vocab_parallel_source_probs, vocab_parallel_target_topk_probs, \
            vocab_parallel_target_topk_indices = ctx.saved_tensors
        ...
        arange_1d = torch.arange(start=0, end=grad_input_2d.size(0), 
                                 device=grad_input_2d.device) # (b*s, )
