...
| `async_training.checkpoint_engine.active`| Whether to use checkpoint_engine for accelerating, default `True`|
| `async_training.checkpoint_engine.enable_broadcast_update_pipeline` | When use checkpoint_engine, whether to enable pipeline between broadcast and load_weights, default `False`|
...
* `async_training.checkpoint_engine.device_buffer_size_M`
  ...
  * When enable `enable_broadcast_update_pipeline`, the additional device memory overhead of 
    trainer rank is `3 * bucket_size`and rollout rank is `2 * bucket_size`。
  * When disable `enable_broadcast_update_pipeline`, the additional device memory overhead of 
