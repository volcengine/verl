...
python3 -m verl.trainer.main_ppo --config-path=config \
    ...
    actor_rollout_ref.ref.megatron.virtual_pipeline_model_parallel_size=4 \
    actor_rollout_ref.ref.megatron.tensor_model_parallel_size=4 \
    critic.optim.lr=2e-5 \
    critic.model.path=$HOME/models/deepseek-llm-7b-chat \
    critic.model.enable_gradient_checkpointing=False \
    critic.ppo_micro_batch_size_per_gpu=4 \
    critic.megatron.pipeline_model_parallel_size=4 \
    critic.megatron.virtual_pipeline_model_parallel_size=4 \
