...
class SimpleAtroposClient:
    ...
    def get_trajectory_batch(
        ...
        inference_endpoint: str = "http://localhost:9000"
    ) -> TrajectoryBatch:
        ...
        return self.real_api.get_trajectory_batch(
            batch_size=batch_size,
            environment=environment,
    ...
    def _combine_trajectory_batches(self, batches: List[TrajectoryBatch]) -> TrajectoryBatch:
        ...
        combined_metadata = {
            ...
            "batch_sizes": [len(batch.tokens) for batch in batches],
            "total_batch_size": len(combined_tokens)
        }
...
class SimplifiedGRPOTrainer:
    ...
    def get_training_batch(self) -> TrajectoryBatch:
        ...
        return self.atropos_client.get_multi_environment_batch(
            total_batch_size=self.batch_size,
            environment_weights=self.environment_weights,
            inference_endpoint=self.inference_manager.get_endpoint()
        )
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for step in range(steps_per_epoch):
            ...
            if step % 5 == 0 or step == steps_per_epoch - 1:
                env_info = ", ".join([f"{env}: {batch_data.metadata['batch_sizes'][i]}" 
                                    for i, env in enumerate(batch_data.metadata['source_environments'])])
                logger.info(f"  Step {step + 1:2d}/{steps_per_epoch}: "
                          f"loss={step_metrics['loss']:.3f}, "
                          f"reward={step_metrics['reward']:.3f}, "
                          f"envs=[{env_info}]")
