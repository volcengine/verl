...
from verl.utils.atropos_utils import (
    AtroposDataConverter, get_device_for_tensor_ops,
    AtroposAPIValidator
)
...
def test_atropos_data_conversion(tokens, masks, scores, device):
    ...
    atropos_data = AtroposDataConverter.verl_to_atropos_batch(
        tokens=tokens,
        masks=masks, 
        scores=scores
    )
    ...
    mock_batch = {
        'batch': [
            {
                'tokens': atropos_data['tokens'][0],
                'masks': atropos_data['masks'][0],
                'advantages': [0.5] * len(atropos_data['tokens'][0]),
                'scores': [atropos_data['scores'][0]]
            },
            {
                'tokens': atropos_data['tokens'][1],
                'masks': atropos_data['masks'][1], 
                'advantages': [0.3] * len(atropos_data['tokens'][1]),
                'scores': [atropos_data['scores'][1]]
            }
        ]
    }
    
    # Test Atropos to VeRL conversion (should move to target device)
    converted_tokens, converted_masks, converted_advantages, converted_scores = \
        AtroposDataConverter.atropos_to_verl_batch(mock_batch, device=device)
    
    print(f"   Converted back to VeRL format")
