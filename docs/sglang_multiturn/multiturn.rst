...
2. Apply the chat template again including the latest message (`messages[:i+1]`) and remove contents after the last EOS token (since apply_chat_template() usually appends a '\\n' after EOS). For more information, please refer to force_chat_end_with_eos() in schemas.py_.
...
.. code-block:: python
   ...
   curr = force_chat_end_with_eos(tokenizer.apply_chat_template(messages[:i+1], add_generation_prompt=False, tokenize=False),eos_token=tokenizer.eos_token)
...
.. code-block:: python
   ...
   curr = force_chat_end_with_eos(processor.apply_chat_template(messages[:i+1], add_generation_prompt=False, tokenize=False),eos_token=processor.tokenizer.eos_token)
...
.. _schemas.py: https://github.com/volcengine/verl/blob/main/verl/workers/rollout/schemas.py
