...
- `actor_rollout_ref.model.lora_adapter_path`: string, path to a pretrained LoRA adapter. If provided, loads existing adapter instead of creating new one. This enables multi-stage training where you can continue training from a previously saved LoRA adapter.
