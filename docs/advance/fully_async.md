...
| `async_training.recompute_old_log_prob`       | Whether to recompute rollout_log_prob using the training model's parameters during the training phase. |                                                |
...
* `async_training.recompute_old_log_prob` (experimental)
  ...
  technique for importance sampling. To utilize Rollout Importance Sampling, we need to recompute old_log_prob using
  the training engine, which requires enabling this switch.
  Additionally, when recompute_old_log_prob and Rollout Importance Sampling are enabled under mode d
