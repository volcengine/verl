...
if __name__ == "__main__":
    ...
    for path in pathlist:
        ...
        with open(path_in_str, encoding="utf-8") as f:
            ...
            print(
                f"[CHECK] File {path_in_str} is detected for device api usage check, check result: {'success' if not find_invalid_device_management else f'failed, because detect {sk}'}."
            )

            assert not find_invalid_device_management, (
                f'file {path_in_str} contains .cuda/"cuda"/"nccl" usage, please use api in verl/utils/device.py directly.'
            )
