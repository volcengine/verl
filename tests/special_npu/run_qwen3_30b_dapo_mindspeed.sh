...
ray start --head --port=6666 --dashboard-port=8888 --ray-debugger-external
...
if [[ ${USE_DIST_CKPT} == "True" ]]; then
    torchrun --nproc_per_node 4 --nnodes 1 scripts/converter_hf_to_mcore.py \
        --hf_model_path ${MODEL_PATH} \
        --output_path ${DIST_CKPT_PATH}
fi
export RAY_DEBUG_POST_MORTEM
