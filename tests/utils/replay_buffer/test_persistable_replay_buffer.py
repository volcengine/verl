...
class TestPersistableReplayBuffer(unittest.TestCase):
    ...
    def test_write_eviction(self):
        ...
        # Pushing a new key mapping to another batch2_list. The total bytes in dict >45000. Should evict rollout_1 to rocksdb.
    ...
    def test_get_eviction(self):
        ...
        # Get 'rollout_1' -> 'rollout_2' should be evicted to rocksdb and 'rollout_1' should exist in both in-memory cache and rocksdb.
        client.get("rollout_1")
    ...
    def test_large_scale_eviction(self):
        ...
        labels = [random.choice(["abc", "cde"]) for _ in range(100)]
        batch2 = DataProto.from_dict(tensors={"obs": obs}, non_tensors={"labels": labels})
        from verl.utils.replay_buffer.persistable_replay_buffer_util.lru_cache import get_batch_list_bytes
