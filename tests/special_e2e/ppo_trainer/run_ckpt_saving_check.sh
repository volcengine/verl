...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.save_after_epochs="${SAVE_AFTER_EPOCHS}" \
    trainer.save_best_val_metric="${SAVE_BEST_VAL_METRIC}" \
    | tee "${output_file}"


if [ "${SAVE_BEST_VAL_METRIC}" != -1 ]; then
...
if [ "${SAVE_AFTER_EPOCHS}" != "None" ]; then
