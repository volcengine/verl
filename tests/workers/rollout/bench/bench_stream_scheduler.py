...
# config.actor_rollout_ref.rollout.chat_scheduler.micro_batch.max_inflight_req = 256+128
...
config.actor_rollout_ref.rollout.top_p = 1
...
batch_size = 1024
dataloader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=False, drop_last=True, collate_fn=collate_fn)
...
print(f"time cost for each epoch for stream scheduler : {epoch_times}")
# compare:
print(f"stream cost time: {sum(epoch_times)}, native cost time: {sum(native_epoch_times)}")
print(f"stream cost time: {batch_time}, native cost time: {native_batch_cost}")
